...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit_exercise_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE exercises SET question_text = ?, option_one = ?, option_two = ?, option_three = ?, option_four = ?, correct_option = ?, updated_at = NOW() WHERE exercise_id = ?");
    $stmt->bind_param("ssssssi", $question_text, $option_one, $option_two, $option_three, $option_four, $correct_option, $exercise_id);
